...
Reads:
    grading_meta.json   — must match the train run (indices + normalization)
    rl_grading_agent.zip
...
META = ROOT / "grading_meta.json"
MODEL_ZIP = ROOT / "rl_grading_agent.zip"
...
def main() -> None:
    ...
    if not META.is_file():
        raise FileNotFoundError(f"Missing {META.name}. Run train_grading_agent.py first.")
    ...
    lines.append("Rows: `qry6114_2025f_autograding_final.xlsx`, **`questions_id != 9`**. ")
